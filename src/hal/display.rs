...
fn send_u8<T: SpiDevice>(spi: &mut T, words: DataFormat<'_>) -> Result<(), T::Error> {
    match words {
        ...
        DataFormat::U16LE(slice) => {
            use byte_slice_cast::*;
            for v in slice.iter_mut() { *v = v.to_le(); }
            spi.write(slice.as_byte_slice())
        }
        DataFormat::U16BE(slice) => {
            use byte_slice_cast::*;
            for v in slice.iter_mut() { *v = v.to_be(); }
            spi.write(slice.as_byte_slice())
        }
        DataFormat::U8Iter(iter) => {
            ...
            for v in iter.into_iter() {
                buf[i] = v;
                i += 1;
                if i == buf.len() { spi.write(&buf)?; i = 0; }
            }
            if i > 0 { spi.write(&buf[..i])?; }
            Ok(())
        }
        DataFormat::U16LEIter(iter) => {
            ...
            for v in iter.map(u16::to_le) {
                buf[i] = v;
                i += 1;
                if i == buf.len() { spi.write(buf.as_byte_slice())?; i = 0; }
            }
            if i > 0 { spi.write(buf[..i].as_byte_slice())?; }
            Ok(())
        }
        DataFormat::U16BEIter(iter) => {
            ...
            for v in iter.map(u16::to_be) {
                buf[i] = v;
                i += 1;
                if i == len { spi.write(buf.as_byte_slice())?; i = 0; }
            }
            if i > 0 { spi.write(buf[..i].as_byte_slice())?; }
            Ok(())
        }
